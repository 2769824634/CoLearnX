...
export default function AdminLaterApprovalsPage({ type }) {
  ...
  return <section className="admin-review-page later-page"><header className="admin-review-header"><div><p className="admin-form-eyebrow">Governed queue</p><h1>{heading}</h1><p>{subtitle}</p></div><button className="btn btn-ghost" type="button" disabled={query.loading} onClick={query.refresh}>Refresh</button></header>
    ...
    {query.data?.length ? <div className="later-review-list">{query.data.map((item) => {
      const id = item.id || item.versionId;
      return <article className="later-review-card" key={id}><div className="later-review-card-main"><span className="admin-status">{item.status}</span><h2>{type === 'materials' ? item.title : item.learnerName}</h2><p>{type === 'materials' ? `${item.creatorName} · ${item.courseCode || 'Course'} · ${item.format} · version ${item.versionNumber}` : `${item.courseCode} · ${item.courseTitle}`}</p>{type === 'materials' ? <button type="button" className="btn btn-ghost btn-sm" onClick={() => adminLaterPhaseApi.downloadMaterial(token, item.versionId, `${item.title}.${String(item.format || 'bin').toLowerCase()}`).catch((error) => setMutationError(error))}>Open submitted file</button> : <small>Trainer note: {item.trainerReviewReason || 'Approved without a note'}</small>}</div><div className="later-review-card-action"><label htmlFor={`review-note-${type}-${id}`}>Review note / required rejection reason</label><textarea id={`review-note-${type}-${id}`} maxLength="512" value={notes[id] || ''} onChange={(event) => setNotes({ ...notes, [id]: event.target.value })} /><div className="trainer-actions"><button className="btn btn-primary" type="button" disabled={Boolean(busyId)} onClick={() => review(item, 'Approve')}>{busyId === id ? 'Saving…' : 'Approve'}</button><button className="btn btn-danger" type="button" disabled={Boolean(busyId) || !notes[id]?.trim()} onClick={() => review(item, 'Reject')}>Reject</button></div></div></article>;
